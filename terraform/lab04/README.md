## 1. Create a VPC with vpc module.Create an EKS cluster using the eks module with two self-hosted ‘t3.small’ node.
```YAML
module "vpc" {
...
```
## 2. Use Kubernetes provider to create a namespace into EKS called ‘nginx’.
```YAML
...
## 3. Use Helm provider to install the kubernetes-nginx chart to the nginx namespace. Port-forward nginx to localhost to check if it works.
```YAML
...
```
```bash
~/Work/TAP/terraform/lab04$ kubectl port-forward service/nginx 8080:80 --namespace=nginx
